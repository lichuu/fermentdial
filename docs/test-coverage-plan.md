...
output with no cheap host-side capture path, so that split leans on compile+link plus the
manual UI smoke checklist below. The gap is real; it's called out here so nobody mistakes it
for covered.
...
This is the highest-value, most durable asset, and it directly de-risks the
`config.h → programs.h` split. All target logic in `config.h` is header-only `inline`, so
these tests compile just `config.h` against a tiny shim: no firmware `.cpp`, no hardware.

In `platformio.ini`, add a host env (it must override the global `[env]` espressif defaults):
```ini
[env:native]
platform = native
...
```
`test_build_src` stays at its default (`no`) for the settings suite, since it's header-only.

`test/mocks/Arduino.h` is a minimal shim; its surface is mapped exactly from what
`config.h` uses:
...
`test/test_settings/test_settings.cpp` does `#include "config.h"` and runs Unity cases on
the pure helpers (signatures confirmed pure, no String I/O unless noted):
- `sanitizeSettings` (config.h:793), as characterization: feed a deliberately out-of-range
...
**before** starting the refactor; the baseline must come from pre-refactor firmware. The demo
env populates simulated sensor/hydrometer data and forces physical outputs off, so endpoints
return full payloads without real hardware. Auth note: when no admin password is set,
`isAuthed()` returns true (network.cpp:1037) and all endpoints are open; the script handles the
password-set case by logging in (`POST /login`, capture `fdsession` cookie) for the two
auth-gated reads.

`test/golden/capture.sh <host>` curls each endpoint, normalizes, and writes to an output dir:
...
- **Skip** `/api/history`, `/api/history.csv`, `/api/events`, `/api/selfcheck`: fully volatile,
  low format-regression risk, high diff noise. The skip is logged, not silent.
...
; `diff -ru before after`. An empty diff means HTTP behavior is preserved.
...
These tests pin the CLAUDE.md safety contract directly: heater+pump never both energized,
sensor-fault forces outputs off, demo mode never drives physical pins. `FermentationController`
is well-factored for this: time is fully injected via `nowMs` (no internal `millis()`), and the
...
Add a second env to `platformio.ini` for the demo-guard build (the demo output suppression is
...
the non-demo control suite too. (The settings suite ignores the extra TU.)

The stub headers `test/mocks/OneWire.h` and `test/mocks/DallasTemperature.h` are inline no-op
classes covering exactly the methods `TemperatureSensor` calls (enumerate with
...
`test/test_control/test_control.cpp` covers interlock/fault under `native` and suppression
under `native_demo`:
...
- **Demo build:** drive `update()` so the logic wants the heater on; assert `heaterOn()` reflects
  the *logical* decision **but** `mock_gpio[PIN_HEATER_TRIGGER]` stayed at the off level, i.e.
  the physical pin was never energized (control.cpp:244).
...
3. **Don't perturb firmware builds:** the test additions must not break the device envs, so run
   the full all-env build from CLAUDE.md:
   ```sh
   UV_CACHE_DIR="$PWD/.uv-cache" \
   PLATFORMIO_HOME_DIR="$PWD/.platformio" \
...
If all nine pass, the UI change is verified for this plan. Any failure is a regression until
fixed or the checklist is updated deliberately.
...
- **Render golden tests:** framebuffer snapshot comparison would require on-device capture or
  an emulator. Out of scope; the manual smoke above remains the render net.
